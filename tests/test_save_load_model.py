...
def test_save_query_get_first():
    ...
    st.save()
    sleep(1)
    st2 = Student.objects.filter(auth_info__email=data['auth_info']['email'])[0]
    clean_value = st2.clean_value()
    clean_data['timestamp'] = clean_value['timestamp']
    assert clean_data == clean_value

def test_save_query_list_results():
    Student.objects._clear_bucket()
    st = Student()
    st._load_data(data)
    st.save()
    sleep(1)
    students = list(Student.objects.filter(auth_info__email=data['auth_info']['email']))
    st2 = students[0]
    clean_value = st2.clean_value()
    clean_data['timestamp'] = clean_value['timestamp']
    assert clean_data == clean_value
